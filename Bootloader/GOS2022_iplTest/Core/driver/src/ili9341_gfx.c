...
static void ili9341_fill_quarter_circle(ili9341_color_t color,
    ...
    uint16_t color[]/* already byte-swapped (LE) */, ili9341_bool_t wait)
{
  ...
  //spi_driver_transmit_dma((uint8_t *)color, size);
...
void ili9341_draw_circle(ili9341_color_t color,
    int16_t x, int16_t y, int16_t r)
{
  ...
  ili9341_draw_pixel(color, x, y + r);
  ili9341_draw_pixel(color, x, y - r);
  ili9341_draw_pixel(color, x + r, y);
  ili9341_draw_pixel(color, x - r, y);
  ...
  while (ix < iy) {
    ...
    ili9341_draw_pixel(color, x + ix, y + iy);
    ili9341_draw_pixel(color, x - ix, y + iy);
    ili9341_draw_pixel(color, x + ix, y - iy);
    ili9341_draw_pixel(color, x - ix, y - iy);
    ili9341_draw_pixel(color, x + iy, y + ix);
    ili9341_draw_pixel(color, x - iy, y + ix);
    ili9341_draw_pixel(color, x + iy, y - ix);
    ili9341_draw_pixel(color, x - iy, y - ix);
  }
...
void ili9341_fill_circle(ili9341_color_t color,
    int16_t x, int16_t y, int16_t r)
{
    ...
    int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *bmp)
{
  ...
  // select target region
  ili9341_spi_tft_set_address_rect(x, y, w, h);
  ili9341_spi_tft_select();
  ...
  for (int16_t j = 0; j < h; ++j, ++y) {
    ...
    ili9341_transmit_wait();
    ili9341_transmit_color(w * 2, &(spi_tx_block[(j&1) * w]), ibNo);
  }
...
void ili9341_draw_string(ili9341_text_attr_t attr, char str[])
{
	//if (gos_mutexLock(&ili9341Mutex, 1000) == GOS_SUCCESS){
  ...
  while (ix < iy) {
    ...
    if (ix < (iy + 1)) {
      if (corners & 1) {
        tx = x + ix;
        ty = y - iy;
        ili9341_draw_line(color, tx, ty, tx, ty + 2 * iy + delta);
      }
      if (corners & 2) {
        tx = x - ix;
        ty = y - iy;
    ...
    if (iy != py) {
      if (corners & 1) {
        tx = x + py;
        ty = y - px;
        ili9341_draw_line(color, tx, ty, tx, ty + 2 * px + delta);
      }
      if (corners & 2) {
        tx = x - py;
        ty = y - px;
