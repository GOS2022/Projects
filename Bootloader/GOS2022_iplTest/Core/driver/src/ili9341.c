...
#define __SLAVE_SELECT(s)  \
  if (__IS_SPI_SLAVE(s)) { ili9341_spi_slave_select((s)); }

#define __SLAVE_RELEASE(s) \
  if (__IS_SPI_SLAVE(s)) { ili9341_spi_slave_release((s)); }
		...
		ili9341_touch_normalize_t touch_normalize)
{
  ...
  //if (NULL != spi_hal) {
    ...
    if ( //(NULL != reset_port)        && IS_GPIO_PIN(reset_pin)         &&
      ...
      if ( 1){//itsSupported != touch_support){ //||
...
void ili9341_touch_interrupt()
{
  uint16_t x_pos;
  uint16_t y_pos;

  // read the new/incoming state of the touch screen
  ili9341_touch_pressed_t pressed = 0;
...
ili9341_touch_pressed_t ili9341_touch_pressed()
{
  ...
  //if (__GPIO_PIN_CLR__ == HAL_GPIO_ReadPin(lcd.touch_irq_port, lcd.touch_irq_pin)) // TODO
    { return itpPressed; }
  //else
  //  { return itpNotPressed; }
}

void ili9341_set_touch_pressed_begin(ili9341_touch_callback_t callback)
{
...
void ili9341_set_touch_pressed_end(ili9341_touch_callback_t callback)
{
  if (/*(NULL != lcd) &&*/ (NULL != callback)) {
...
#if 0
ili9341_touch_pressed_t ili9341_touch_coordinate(uint16_t *x_pos, uint16_t *y_pos)
{
  //if (NULL == lcd)
  //  { return itpNONE; }
  ...
  spi_driver_changeSpeed(SPI_BAUDRATEPRESCALER_128);
  ...
  while ((itpPressed == ili9341_touch_pressed()) && (sample--)) {
    ...
    spi_driver_transmit_blocking((uint8_t*)x_cmd, sizeof(x_cmd));
    //HAL_SPI_TransmitReceive(lcd->spi_hal, (uint8_t*)x_cmd, x_raw, sizeof(x_raw), __SPI_MAX_DELAY__);
    spi_driver_transmit_receive_blocking((uint8_t*)x_cmd, x_raw, sizeof(x_raw));

    //HAL_SPI_Transmit(lcd->spi_hal, (uint8_t*)y_cmd, sizeof(y_cmd), __SPI_MAX_DELAY__);
    spi_driver_transmit_blocking((uint8_t*)y_cmd, sizeof(y_cmd));
    //HAL_SPI_TransmitReceive(lcd->spi_hal, (uint8_t*)y_cmd, y_raw, sizeof(y_raw), __SPI_MAX_DELAY__);
    spi_driver_transmit_receive_blocking((uint8_t*)y_cmd, y_raw, sizeof(y_raw));
  ...
  spi_driver_transmit_blocking((uint8_t*)sleep, sizeof(sleep));
  ...
  spi_driver_changeSpeed(SPI_BAUDRATEPRESCALER_4);

  if (num_samples < req_samples)
    { return itpNotPressed; }

  ili9341_two_dimension_t coord =
      ili9341_project_touch_coordinate(x_avg / req_samples, y_avg / req_samples);
...
void ili9341_calibrate_scalar(uint16_t min_x, uint16_t min_y, uint16_t max_x, uint16_t max_y)
{
  //if (NULL == lcd)
  //  { return; }
...
void ili9341_spi_write_data_read(
    ...
    uint16_t data_sz, uint8_t tx_data[], uint8_t rx_data[])
{
  ...
  //spi_driver_transmit_receive_blocking(tx_data, rx_data, data_sz);
  drv_spiTransmitReceiveBlocking(DRV_SPI_INSTANCE_3, tx_data, rx_data, data_sz, /*GOS_MUTEX_ENDLESS_TMO*/3000, 1000);
  //drv_spiTransmitReceiveDMA(DRV_SPI_INSTANCE_3, tx_data, rx_data, data_sz, GOS_MUTEX_ENDLESS_TMO, 1000);
